            ...
            <tbody>
              {users.map((user) => (
                <tr key={user._id}>
					...
					<td>
						<Link to={`/admin/edit/user/${user._id}`} className="text-blue-500 hover:text-coral-red">
						Edit
						</Link>
					</td>
					<td>
						<i className='bx bx-trash text-coral-red cursor-pointer' onClick={() => handleDelete(user._id)} ></i>
					</td>
                </tr>
              ))}
            </tbody>
